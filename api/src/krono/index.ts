...
import { shuffle, partition, sortBy, groupBy, cloneDeep, omit } from "lodash";

export type Action =
  | { type: "INIT"; playerIds: string[]; botCount: number }
  | {
...
const recipe = (state: State, action: Action): void => {
  const previousState = cloneDeep({ ...state, histories: [] });

  if (action.type === "INIT") {
    const botIds = Array.from({ length: action.botCount }).map(() => ulid());
    const allIds = [...action.playerIds, ...botIds];
    if (allIds.length > MAX_PLAYER_COUNT) {
      throw "invalid";
    }

    state.players = allIds.map((id) => {
      const cards = shuffle(
        [
    ...
    switch (action.type) {
      ...
      case "END_TURN": {
        ...
        if (
          ...
          if (
            state.players.filter((p) => p.coronationCeremonyDeclared).length ===
            1
          ) {
  ...
  state.histories.push({ action, state: previousState });

  handleBotAction(state);
};

const handleBotAction = (state: State) => {
  const botIds = state.players.filter((p) => p.isBot).map((p) => p.id);

  Object.entries(groupBy(state.awaitingActions, (a) => a.playerId))
    .filter(([id]) => botIds.includes(id))
    .forEach(([id, actions]) => {
      if (actions[0].type === "HAND_ELIMINATION") {
        recipe(state, {
          type: "HAND_ELIMINATION",
          playerId: id,
          coin: 5,
        ...
        if (actions.some((a) => a.type === "END_TURN")) {
          recipe(state, { type: "END_TURN", playerId: id });
        }
      }
    });
};

export const reducer = produce(recipe);
